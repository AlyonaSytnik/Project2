...
>>> 0000 0000 0000 0001
    ...
    0000 0000 0000 0005
```
### В модуле **decorators** представлен декоратор ```log```.
...
### В модуле **utils** представлены 6 функций: ```financial_transactions```, ```get_transactions_by_state```, ```find_transactions_with_search_word```, ```count_transactions_with_categories```, ```get_transactions_by_currency```,  ```get_sorted_transactions_by_date```
...
В остальных функциях реализовано считывание финансовых операций из CSV- и XLSX-файлов.

### В модуле **external_api* представлены 2 функции: ```convert_to```, ```convert_to_rub```.

Функция ```convert_to``` ринимает значение (value, тип float), исходный код валюты (from_currency, тип str) и желаемый код валюты
